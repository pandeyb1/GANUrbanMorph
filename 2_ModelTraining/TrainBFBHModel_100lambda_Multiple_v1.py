    ...
    def __init__(self, input_dir, target_dir):
        ...
        for filename in self.input_files:
            ...
            with rasterio.open(target_path) as src:
                ...
                target_image = target_image/3.280839895
...
class GeneratorLoss(nn.Module):
    def __init__(self, alpha=100):
        super().__init__()
        self.alpha = alpha
        self.adversarial_loss_fn = nn.MSELoss()  # Use MSELoss for adversarial loss
        self.l1 = nn.L1Loss()

    def forward(self, fake, real, fake_pred):
        fake_target = torch.ones_like(fake_pred)
        # Calculate adversarial loss using MSELoss
        adversarial_loss = self.adversarial_loss_fn(fake_pred, fake_target)
        # Calculate L1 loss
        l1_loss = self.l1(fake, real)
        # Total loss is the sum of adversarial and weighted L1 loss
        loss = adversarial_loss + self.alpha * l1_loss
        return loss
...
dataloader = DataLoader(dataset, batch_size=batchsize, shuffle=True, pin_memory=True,num_workers=12) ## Num_workers can lead to a broken pipe if used incorrectly. Set to 0.
...
for lr in learning_rates: # Loop through each learning rate
    ...
    for epoch in range(num_epochs):
        ...
        for i, (input_images, target_images) in enumerate(dataloader):
            ...
            fake = fake.detach() # Pass latent_vector to generator
            fake_pred = discriminator(fake, cond)
        ...
        if (epoch+1) % 500 == 0 or (epoch+1) == num_epochs: # Save at the last epoch as well
             torch.save(generator.state_dict(), f'/Users/9oy/Documents/Projects/IM3/EvaluationP/BFCond_BHgenerator_withLatentVector_100L_epoch_{epoch+1}_LR_{lr}.pth')
    ...
    np.savetxt(f'/Users/9oy/Documents/Projects/IM3/EvaluationP/BFCond_BHgenerator_withLatentVector_100L_LR_{lr}_GLOSS.csv', np.array(GLOSS), delimiter=',')
    np.savetxt(f'/Users/9oy/Documents/Projects/IM3/EvaluationP/BFCond_BHgenerator_withLatentVector_100L_LR_{lr}_DLOSS.csv', np.array(DLOSS), delimiter=',')

# Load the saved generator model
generator_path = '/Users/9oy/Documents/Projects/IM3/EvaluationP/BFCond_BHgenerator_withLatentVector_100L_epoch_1000_LR_0.0001.pth'
