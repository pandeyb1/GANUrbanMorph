...
os.environ.setdefault("MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "matplotlib-maintext-morans-error-bias"))
os.environ.setdefault("XDG_CACHE_HOME", str(Path(tempfile.gettempdir()) / "maintext-morans-error-bias-cache"))
...
DEFAULT_TABLE = SPATIAL_DIR / "tables" / "table_morans_i_summary_epoch1000.csv"
DEFAULT_OUT = THIS_DIR / "figures" / "morans_i_error_bias_scatter_epoch1000_unet_single_latent.png"
...
PANEL_SPECS = [
    {
        "target_label": "BF",
        "title": "Building Footprint: Spatial clustering",
        "xlabel": "MAE in Moran's I",
        "ylabel": "MB in Moran's I",
    },
    {
        "target_label": "BH",
        "title": "Building Height: Spatial clustering",
        "xlabel": "MAE in Moran's I",
        "ylabel": "MB in Moran's I",
    },
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a compact Moran's I MAE-vs-MB figure for U-Net and single-latent cGAN at epoch 1000."
    )
    parser.add_argument("--table", default=str(DEFAULT_TABLE), help="Path to table_morans_i_summary_epoch1000.csv")
    ...
    parser.add_argument("--copy-to-spatial-results", action=argparse.BooleanOptionalAction, default=True)
...
def load_frame(table_path: Path, families: list[str], checkpoint: int) -> pd.DataFrame:
    if not table_path.exists():
        raise FileNotFoundError(f"Missing Moran's I table: {table_path}")
...
def add_panel(ax: Any, frame: pd.DataFrame, spec: dict[str, str]) -> None:
    ...
    ax.set_xlim(*padded_limits(panel["moran_mae"].to_numpy(dtype=float), include_zero=True))
    ax.set_ylim(*padded_limits(panel["moran_mbe"].to_numpy(dtype=float), include_zero=True))
...
def save_figure(frame: pd.DataFrame, out_path: Path) -> None:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(9.8, 4.9), constrained_layout=False)
    for ax, spec in zip(axes.flat, PANEL_SPECS, strict=True):
        add_panel(ax, frame, spec)
    fig.subplots_adjust(left=0.095, right=0.985, top=0.89, bottom=0.26, wspace=0.28)
    add_legend(fig)
...
def main() -> int:
    ...
    if args.copy_to_spatial_results:
        copy_path = SPATIAL_FIGURE_DIR / out_path.name
